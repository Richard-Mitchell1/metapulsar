...
def build_ipta_model(psr, inc_ecorr=True):
    ...
    efeq_sel = selections.Selection(
        create_selection_stag("efacequad", {("group", "f"): None})
    )
    ...
    efeq = white_signals.MeasurementNoise(
        efac=efac, log10_t2equad=equad, selection=efeq_sel, name="white"
    )
    ec = white_signals.EcorrKernelNoise(
        log10_ecorr=ecorr, selection=ecorr_sel, name="ecorr"
    )

    dm = gp_signals.BasisGP(
        dm_pl, dm_basis, name="dm_gp", coefficients=False, selection=no_sel
    )
...
def main():
    ...
    have_ecorr = (
        np.sum(metapsr.flags["pta"] == "PPTA")
        + np.sum(metapsr.flags["pta"] == "NANOGrav")
    ) > 0
